---
title: "Descriptive analysis by church"
...
addresses <- readxl::read_xlsx("C:/Users/fallouch/Box/PhD/CHERISH/CHERISH/Needs Assessment Survey Addresses.xlsx")

addresses <- addresses %>% 
  select(-c("...6", "...7", "...8")) %>% 
  mutate(Chruch = ifelse(Chruch == "St. Raymond and St. Leo the Great" |
                           Chruch == "St. Raymond/ St. Leo the Greatat" |
                           Chruch == "St. Raymond/St. Leo the Great", "St. Raymond/ St. Leo the Great", Chruch)) %>% 
  rename(record_id = "Prticipant ID",
         church = Chruch,
         street = Street,
         city = "City, State",
         zip_code = "Zip Code")

cherish <- merge(cherish, addresses, by = "record_id")
...
```{r}
cherish %>%
  filter(church == "All Saints Catholic Church") %>% 
...
```{r}
cherish %>%
  filter(church == "Central Missionary Baptist") %>% 
  select(q28, q29.factor, q30.factor, q31.factor, q32.factor, q33.factor, q34.factor) %>% 
  tbl_summary(digits = ~ c(1)) %>% 
  bold_labels()
```
...
\newpage
# Corinne Missionary Baptist
## Demographics
```{r}
cherish %>%
  filter(church == "Corinne Missionary Baptist") %>% 
  select(q28, q29.factor, q30.factor, q31.factor, q32.factor, q33.factor, q34.factor) %>% 
  tbl_summary(digits = ~ c(1)) %>% 
  bold_labels()
```
...
```{r}
cherish %>%
  filter(church == "Greater Liberty Baptist Church") %>% 
  select(q28, q29.factor, q30.factor, q31.factor, q32.factor, q33.factor, q34.factor) %>% 
  tbl_summary(digits = ~ c(1)) %>% 
  bold_labels()
```
...
```{r}
cherish %>%
  filter(church == "New Hope Baptist Church") %>% 
  select(q28, q29.factor, q30.factor, q31.factor, q32.factor, q33.factor, q34.factor) %>% 
  tbl_summary(digits = ~ c(1)) %>% 
  bold_labels()
```
...
```{r}
cherish %>%
  filter(church == "St. Maria Goretti") %>% 
  select(q28, q29.factor, q30.factor, q31.factor, q32.factor, q33.factor, q34.factor) %>% 
  tbl_summary(digits = ~ c(1)) %>% 
  bold_labels()
```
...
```{r}
cherish %>%
  filter(church == "St. Mary of the Angels") %>% 
  select(q28, q29.factor, q30.factor, q31.factor, q32.factor, q33.factor, q34.factor) %>% 
  tbl_summary(digits = ~ c(1)) %>% 
  bold_labels()
```
...
```{r}
cherish %>%
  filter(church == "St. Raymond/ St. Leo the Great") %>% 
  select(q28, q29.factor, q30.factor, q31.factor, q32.factor, q33.factor, q34.factor) %>% 
  tbl_summary(digits = ~ c(1)) %>% 
  bold_labels()
```
